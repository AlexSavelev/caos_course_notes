...
## Условные конструкции и переходы
```cpp
int sum(int a) {
	if (a > 25) {
		e++;
...
# Связывание ассемблерного кода с C++
## Линкуем assembler'ный код
```nasm
; is_prime.asm:
...
# Вызов функций: `call`, `ret`, стековый фрейм
...
### Про флаг `-fno-omit-frame-pointer`
Эта опция компилятора запрещает использовать `rbp` как регистр общего назначения, сохраняя его как указатель на фрейм. Упрощает отладку, но может немного замедлить код.


## Передача аргументов: System V ABI
...
**Все это было лишь каплей в море**. На деле инструкций в современных процессорах ОЧЕНЬ МНОГО. Поэтому в 21 веке писать на ассебмлебе приходится очень редко, ибо современные компиляторы в 99.99+% случаев знают что делать куда лучше вас.
...
std::cout << b;  // Прочтет снова из стека
```
...
## Измерение тактов процессора (RDTSC)
...
# Timing Attack и константное время
...
# Атака переполнения буфера и защита
...
1. **Stack Protector** (Stack Canary)
   ...
   - Флаги: `-fstack-protector` (по умолчанию), `-fno-stack-protector` для отключения.
