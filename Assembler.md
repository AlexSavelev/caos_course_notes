...
- `push <операнд>`
	- Уменьшает `RSP` (например, на 4 байта в 32-битном режиме), затем помещает значение операнда в память по новому адресу `[RSP]`.
- `pop <операнд>`
	- Берет значение с вершины стека (`[RSP]`) и загружает его в операнд, затем увеличивает `RSP` (освобождая память).
- `leave`
...
```cpp
struct Small { int a, b; };  // Возврат в rdx:rax
