...
Все этого весело, но долго.
- Действительно, без дополнительных оптимизаций с современными многоуровневыми таблицами страниц каждая попытка перевести виртуальный адрес в физический сопровождалось бы несколькими обращениями в медленную RAM.
...
2. Проверяется TLB (TLB Lookup)
	- TLB Hit (Попадание в TLB): Запись найдена! MMU мгновенно формирует физический адрес
    - TLB Miss (Промах в TLB): Нужного отображения в TLB нет. Продолжаем.
3. Теперь MMU выполняет "полный проход" — Page Walk — обращаясь к таблицам страниц в RAM, чтобы найти нужное отображение.
...
7. После успешного поиска MMU загружает эту запись в TLB (возможно, вытеснив какую-то старую).
...
- **iTLB (Instruction TLB):** Кеширует только трансляции для страниц, содержащих машинный код (инструкции) CPU. Используется на этапе Fetch конвейера CPU, когда нужно загрузить следующую инструкцию по виртуальному адресу.
- **dTLB (Data TLB):** Кеширует только трансляции для страниц, содержащих данные, с которыми работают инструкции (чтение/запись из/в память). Используется на этапах Load/Store.

После прочтения статей про Assembler и Low Level Optimization будет понятнее, но разделения TLB на 2 группы связано с:
- Параллелизмом: Конвейер CPU может одновременно выполнять этап Fetch (требующий iTLB) и этап выполнения инструкции чтения/записи (требующий dTLB). Раздельные буферы устраняют конфликты доступа.
- Безопасностью: Разделение косвенно добавляет изоляцию. Ошибка или атака на dTLB не скомпрометирует iTLB.
...
- minor — страница уже в оперативной памяти, но процесс о ней "не знает". ОС просто обновляет таблицы страниц — без дисковых операций. Очень быстро.
- major — обращение к файлу. Да, major page fault возникает, когда страница отсутствует в ОЗУ и должна быть загружена с диска.
	...
	- Например, загрузка кода из диска
...
**Замечание:**
Если выделить 3 страницы (например, 10'000 байт) и вызвать `munmap(1000)`, то это UB — нужно всегда ан-мапить количество, кратное размеру одной страницы (4096). Но скорее всего, за-анмапится ровно 1 страница (то есть нам останется меньше, чем ожидалось).
...
```
- Начиная с какого-то индекса (~ $2^{14} = 16384$), значения адресов заметно поменяются. Это и будет означать, что вектор "переехал с кучи".
...
### 8.1.1 Образовательный пример
Запустить получится не с первого раза, только после отключения многих фич современного Линукса.
...
### 8.1.2 Исполнение процессорных инструкций, записанных на "исполняемой" памяти
```cpp
#include <vector>
#include <list>
#include <iostream>
...
- Строка `v[0] = 0x33;` нужна, чтобы не было Segmentation Fault
- С ней будет Illegal instruction ($\in$ RE)

Illegal Instruction Возникает при выполнении некорректного машинного кода или кода без `PROT_EXEC`.

### 8.1.3 Использование dlopen/dlsym (практический способ)
```cpp
#include <dlfcn.h>
#include <iostream>
