...
# Системные вызовы ввода-вывода (I/O)
...
### `openat` — открытие относительно директории
...
- Возвращает новую позицию или `(off_t)-1` при ошибке
...
## Программа `tee` — практический пример
...
#### Sticky bit (`t` в правах директории):  
...
```
Если `x` отсутствует, отображается `S`.

#### SGID-бит (`s` для группы):  
...
## Метаданные файлов: `stat`, `fstat`, `lstat`
...
### Syscall'ы для работы со ссылками
...
- `dup2` **закрывает** `newfd`, если он был открыт, перед копированием
- `dup3` с флагом `O_CLOEXEC` автоматически закрывает дескриптор при вызове `exec`
...
int dup3(int oldfd, int newfd, int flags); // с флагами (например, O_CLOEXEC)
```
...
### `fcntl` — управление открытыми файловыми дескрипторами
```cpp
#include <fcntl.h>
...
### `ioctl` — универсальный интерфейс управления
