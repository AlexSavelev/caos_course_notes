...
# `/dev` — устройства и виртуальные файлы
...
- `/dev/null` — игнорирует запись, возвращает EOF при чтении
 ...
 - `/dev/random` и `/dev/urandom` — генераторы случайных чисел
	 - `random` блокируется при нехватке энтропии (это обсудим позже, пока на веру это +- одно и то же)
	 - `urandom` — нет (рекомендуется для большинства задач)
...
### Пишем на символьные устройства и читаем с них
...
## 7.1 Именованные пайпы (FIFO)
FIFO — named pipe, существует как файл в файловой системе.

**`mknode` - утилита, позволяющая создавать файлы разных типов:**
```bash
man mknode
```
- Ему соответствует syscall `mknod`.
...
## `/sys` — информация об устройствах и ядре в структурированном виде
